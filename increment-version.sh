...
cat deployment.yaml | sed -re "s/image:[[:space:]]robinhoodis\/melt:.*/image: robinhoodis\/melt:${CONTAINERVERSION}/" > deployment.yaml.tmp && mv deployment.yaml.tmp deployment.yaml
